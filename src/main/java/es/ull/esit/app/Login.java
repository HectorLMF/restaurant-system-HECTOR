...
public class Login extends javax.swing.JFrame {
  ...
  /**
   * @brief Factory (test seam) to create the admin window.
   *
   *        Tests can override it to avoid opening real UI windows.
   */
  transient java.util.function.Supplier<? extends javax.swing.JFrame> adminSupplier = AdminLogin::new;

  /**
   * @brief Factory (test seam) to create the cashier window.
   *
   *        Receives the logged-in username and returns the corresponding frame.
   *        Tests can override it to avoid opening real UI windows.
   *
   * @note The username passed is taken from the authenticated User object.
   */
  transient java.util.function.Function<String, ? extends javax.swing.JFrame> cashierFactory = CashierLogin::new;

  /**
   * @brief Factory (test seam) to create Login instances for main().
   *
  ...
  /**
   * @brief Abstraction for message dialogs (test seam).
   *
   *        Wraps JOptionPane to allow tests to intercept messages
   *        without displaying UI pop-ups.
   */
  interface MessageDialog {
    /**
     * @brief Shows an informational message dialog.
     *
     * @param parent  [java.awt.Component] Parent component for dialog positioning.
     * @param message [Object] Message to display.
     */
    void showMessage(java.awt.Component parent, Object message);

    /**
     * @brief Shows a message dialog with a title and message type.
     *
     * @param parent      [java.awt.Component] Parent component for dialog
     *                    positioning.
     * @param message     [Object] Message to display.
     * @param title       [String] Dialog title.
     * @param messageType [int] JOptionPane message type (e.g., ERROR_MESSAGE).
     */
    void showMessage(java.awt.Component parent, Object message, String title, int messageType);
  }

  /**
   * @brief Dialog handler used to display messages to the user (test seam).
   *
   *        Default implementation delegates to JOptionPane.
   *        Tests can inject a fake implementation to avoid real dialogs.
   */
  transient MessageDialog messageDialog = new MessageDialog() {
    @Override
  ...
  /**
   * @brief Injects the supplier used to create the admin window (test seam).
   *
   * @param s [java.util.function.Supplier<? extends javax.swing.JFrame>] Supplier
   *          to use.
   */
  ...
  /**
   * @brief Injects the factory used to create the cashier window (test seam).
   *
   * @param f [java.util.function.Function<String, ? extends javax.swing.JFrame>]
   *          Factory to use. Receives the username and returns a frame.
   */
  ...
  /**
   * @brief Injects the dialog handler used to show messages (test seam).
   *
   * @param md [MessageDialog] Dialog handler to use.
   */
  ...
  /**
   * @brief Injects the Login factory used by main() (test seam).
   *
   * @param f [java.util.function.Supplier<Login>] Supplier that creates Login
   *          instances.
   */
  ...
  /**
   * @brief Retrieves the Login factory used by main() (test seam).
   *
   * @return [java.util.function.Supplier<Login>] Supplier that creates Login
   *         instances.
   */
   ...
   *        Sets up logo image, main label, input fields for username and
   *        password, combo box for user type, and button for "Log In", along with
   *        the layout and configuration of the window.
   */
